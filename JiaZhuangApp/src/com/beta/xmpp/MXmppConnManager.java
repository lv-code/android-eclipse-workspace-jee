...
public class MXmppConnManager {
	...
	private static String IP = "192.168.9.126";
	...
	public XMPPConnection getConnection() throws XMPPException {
		if (connection == null || !connection.isConnected()) {
			...
			config = new ConnectionConfiguration(IP, PORT, HOST);
			...
			connectionListener = new ReConnectionListener(
					MyBaseApplication.getInstance());
	 ...
	 * @return
	 */
	public boolean mXmppLogin(String name, String pwd, Context context,
			Handler handler) {
		if (connection == null || !connection.isConnected()) {
	 ...
	 */
	/*
	 * public synchronized void sendFile(final long mills,final long
	 * rowid,String type,String file,final Handler handler,final String
	 * toUserId){
	 * 
	 * final MessageDAO messageDAO =
	 * (MessageDAO)MyBaseApplication.getInstance().
	 * dabatases.get(CustomConst.DAO_MESSAGE);
	 * 
	 * Presence presence = connection.getRoster().getPresence(toUserId);
	 * 
	 * final OutgoingFileTransfer transfer =
	 * transferManager.createOutgoingFileTransfer(presence.getFrom());
	 * 
	 * try {
	 * 
	 ...
	 * 
	 * while(!transfer.isDone()){ } messageDAO.updateStateByRowid(rowid,
	 * hostUid, 1); Message msg =
	 * handler.obtainMessage(CustomConst.HANDLER_MSG_FILE_SUCCESS, mills);
	 * handler.sendMessage(msg); MFileTranListener.handRefreshSession(toUserId);
	 * }
	 * 
	 * };
	 * 
